#!/usr/bin/env python3
"""
ff7_limit_menu_static.py -- Statically map the MATERIA menu sub-screen
...
_log_path = os.path.join(
    ...
    f"limit_menu_static_{datetime.datetime.now():%Y%m%d_%H%M%S}.log"
)
...
MATERIA_SUB = 0x70212A  # LIMIT sub = table[7] (row6+1 pattern)     # menu_subs_call_table[3] (v2.31 table dump)
...
STR_VAS = {}
for label in ("Set", "Check", "LEVEL", "Limit level", "Braver"):
...
dump_disasm(MATERIA_SUB, SUB_LEN, "limit_menu_sub (table[7])")
