...
// to 0 AND by TutorialThread when the engine's own TUTORIAL_RUNNING flag
// (0xDBFD30, v2.30.29) drops. proxy.cpp's menu announcement threads
// stand down while set: the tutorial script drives the menu cursor
// itself by INJECTING key presses (the VM overwrites the menu's input
// digests -- disasm 0x71826E, v2.30.29; the 2026-07-26 Materia tutorial
// log's Item->Order sweeps at ~300ms/step were those injections), and
// announcing every scripted step with interrupting speech both clobbers
// the tutorial narration and reads as chaos ("the cursor was jumping
// around"). This latch covers the REQUEST GAP too: TUTOR fires on the
// field side a beat before the menu module raises TUTORIAL_RUNNING, so
// gating on the engine flag alone would let the menu-open announce
// slip through. Plain LONG: x86-atomic, set/clear only.
...
// a separate byte-code script interpreted by the MENU module.
//
// v2.30.27 read the WHOLE lesson up front from the field file's
// akao-offsets table (script format notes: research §8 v2.30.27). Second
// play report: one long buffer of speech, then silence while the player
// guesses how each slide advances. v2.30.29 moves ALL narration to
// TutorialThread (proxy.cpp), which follows the menu module's LIVE
// window state per slide (TUTWIN_* / TUTORIAL_RUNNING in ff7_addresses.h
// — the "unmapped position state" the v1 comment deferred, now mapped by
// ff7_tutorial_static.py). This hook now only flags tutorial-active for
// the menu-thread suppression and logs the request.
// ---------------------------------------------------------------------------
static int __cdecl hook_tutor()
{
    ...
    if (fresh) {
        char dbg[96];
        _snprintf_s(dbg, sizeof(dbg), _TRUNCATE,
            "[FF7Access] TUTOR opcode fired: tutorial index %u",
            FF7Addr::get_opcode_param_byte(0));
        Log::Write(dbg);
    }
    return s_old_tutor();
}
