...
| `MAGICMENU_MODE` | `0x00921100` | u32 magic-screen MODE (audit #4: the v2.30.48 "pane selector 0/1" reading was WRONG and shipped three broken versions .48/.52/.53). LIVE-measured (scoped delta scan v2.30.54 + mode log v2.30.59): **-1 = entering, 0 = tab selector, 2/3/4 = Magic/Summon/E.Skill list (mode = TAB + 2, writes 0x713638/0x713693), 1 = use-on-whom TARGET pane** (v2.30.59 log `mode 2 -> 1` on selecting Cure). cmp sites 0x710E0E/0x710FBA. Magic menu USER-CONFIRMED complete 2026-08-01 |
| `MAGICMENU_TAB` | `0x00DD169C` | Magic/Summon/E.Skill tab selector 0..2, wraps at 3 -- the ONE address that moved in the v2.30.54 delta scan. Each tab owns its own widget + per-character list base: Magic 0xDD1708 / +0x108 / 3 cols, Summon 0xDD1740 / +0x2C8 / 2 cols, **E.Skill 0xDD1778 / +0x348 / 2 cols** (a previously unknown third per-character list; draw 0x710F94/0x71186E/0x711EB8, OK 0x7137A9/0x712C46/0x713284) |
| `MAGICMENU_TARGET_SLOT` | `0x00DD16D4` | party slot 0..2 in the target ("Use on whom?") pane (mode 1); the game's own OK path feeds it to SAVEMAP_PARTY_IDS at 0x7137F8. Play-confirmed v2.30.59 ("spoke characters cleanly") |
...
| `LINE_CROSS_RESULT` | `0x00CC0870` | written per movement step by the player-only LINE crossing test 0x637ABB in the movement tail (test skipped entirely while FIELD_UC_LOCK != 0 -- why LINE triggers never fire during scenes); v2.30.83 ground truth (ff7_player_blocking_ground_truth.py). Not consumed by the mod |
...
| *(countdown tick internals)* | - | v2.30.56 static (ff7_timer_window_static.py): the per-second decrement 0x40AC3C lives in the GLOBAL tick 0x40AB81, gated ONLY by two pause bytes (recorded as savemap+0x1134/+0x1138 = statics 0xDC0E6C/0xDC0E70, just past the 0x10F4 savemap span) and the count-up flag bit (0xDC093B & 2 -> increment branch 0x40AC56) -- **no "timer active" flag exists anywhere**, which is why a leftover value keeps ticking invisibly (the v2.30.8 report class); the honest liveness discriminator is the DISPLAY: WSPCL (0x36) handler 0x61FD5C writes its clock-window type to byte [0xCFF5D3 + win*0x30] (types 1/2 = clock family -- the v2.30.56 second arming signal) and position to +0xE0/+0xE2. Related: 0xDC08B8 play-time seconds; 0xDC08C0/0xDC08C4 = the two x0x444 sub-second accumulators (countdown / play-time) |
...
### v2.30.98 (2026-08-09): battle limit-break selection spoken -- state 24 decoded

**User request** (screenshot BattleScreen/limit_break_selection_1.jpg):
after confirming Limit in battle, the "LIMIT LEVEL 1 / Braver" selection
window was silent -- BMENU state 24 was the last unvoiced battle list
(known since v2.9 as "limit-ish", never decoded).

**Static session** (NEW investigate/ff7_limit_battle_widget_static.py,
log limit_battle_widget_static_20260809_133041.log -- exe file only,
same recipe as the v2.36 submenu session). fn table 0x91E6B8 gave
fn[24] = fn[15] = 0x6DA67E; one listing each of the handler, its
widget-init 0x6DA51A (found adjacent), the draw 0x6DF40D, and the
menu-module builders 0x703517/0x703048 settled everything:

- Widget = WIDGET_BASE + slot*0x700 + **0x118** (0xDC21B8). The init
  pins visible rows = total entries = count, so the list NEVER scrolls
  and the selected index is the RAW vertical cursor w4 -- no w0/scroll
  term, unlike every other battle list.
- Entries = **parallel byte arrays at CHAR_BLOCK+0xAC** (0xDBA544):
  u8 id[3] at +0, u8 target_flags[3] at +3 (confirm: table[w4+3] ->
  0xDC3C84), u8 count at +6 (copied to 0xDC35A4 at init), 3 x 0x1C
  attack records at +8.
- The id IS the 0-based limit index: the game's own draw feeds it to
  get_kernel_text(section 3, id) = the magic-names file at bias +128 --
  byte-for-byte the lookup ResolveActionName branch 7 already performs
  (live-verified v2.30.77: idx 0 = Braver = entry 128). Zero new name
  machinery needed, Aeris/Tifa block swap inherited for free.
- Builders fill ids from the CURRENT limit level (savemap charrec
  +0x0E) via helper 0x5C8684(record, level*3+technique) -- why the
  window lists one level's techniques only.
- fn[24]/draw index their slot from 0xDC3C80 (a second active-slot
  byte); the init uses the known 0xDC3C7C -- equal while open.

**Mod changes (BattleMenuThread only, no cfg change)**: new state-24
branch -- reads w4 + the per-slot count byte (count 1..3 and w4 < count
gate out mid-build reads), resolves via ResolveActionName(0x14, id)
with a positional "limit N" fallback, prefixes "Limit level N." (charrec
+0x0E) on the first announce per list-open (one utterance, the v2.37
turn-prefix lesson), logs "BMENU limit slot= w4= count= id= =>". Also
FIXED IN PASSING: the targeting arm list (from_menu) omitted state 24,
so the post-Confirm target cursor after a single-target limit (Braver)
was silent -- now armed like the other lists.

**Verify queue [LIMITSEL98]** (first battle with a full limit gauge):
(a) confirming Limit speaks "Limit level 1. Braver." and cursor moves
between techniques speak each name (needs 2+ learned -- Cross-slash);
(b) cancel back to the command grid re-announces "Limit", re-entering
re-speaks the header; (c) after confirming Braver the enemy target
cursor SPEAKS (the from_menu fix); (d) a character at limit level 2+
says the right level and techniques; (e) Cait Sith/Tifa limit widgets
(states 26/27, different UIs -- slots/reels) remain out of scope and
should stay silent rather than misspeak; (f) log header v2.30.98.

**Residual**: states 26 (Cait Sith slots) and 27 (Tifa reels) have their
own widgets (handlers 0x6DA898/0x6DA795, dumped in the same log for a
future session); the target_flags byte at +3 is decoded but unused.
...
### v2.30.103 (2026-08-09): prevention rework -- shadow source check, kernel table dump tool, Defend/Change correction

**User directive** (after the .102 shifted-name regression): "Let's
prevent something like that from happening again." Two gaps let .100
ship the wrong convention: the section-5 index was a GUESS that never
appeared in any verify queue, and the play-proven heap source sat next
to the new GKT source with nothing ever comparing them.

**1. New offline tool investigate/ff7_kernel_text_tables_dump.py**
(log kernel_text_tables_dump_*.log): parses KERNEL.BIN's gzip container
directly -- piece numbers and entry indices POSITIONAL, no signature
heuristics (signatures are exactly what hid the filler-entry-0
alignment question). Dumps pieces 10/18-23 with explicit indices.
Byte-results (both installs' KERNEL.BIN identical, 22376 bytes;
2013 path is data\lang-en\kernel\, NOT data\kernel\ which is empty):
  piece 18 (command names) = 32 entries: [0]'Left' FILLER (VANILLA --
  the .102 comments' "Echo-S rebuilds entry 0" attribution was wrong),
  [1]Attack..[4]Item, [5]Steal..[0xD]E.Skill, [0xE]'All:' [0xF]'4x:'
  [0x10]'' [0x11]Mug [0x12]Change [0x13]Defend [0x14]Limit
  [0x15]W-Magic [0x16]W-Sum. [0x17]W-Item [0x18]Slash-All [0x19]2x-Cut
  [0x1A]Flash [0x1B]4x-Cut [0x1C-0x1F]''. Cap 0x20 = byte-exact.
  piece 10 (command descs) = 31 entries, same id-indexed layout
  ([0x12]='Change position...', [0x13]='Defend without attacking').

**2. The dump caught a SECOND dormant instance of the class**:
CommandMenuName's hardcoded cases said 0x12="Defend"/0x13="Change row"
-- REVERSED against both piece 18 AND the project's own v2.30.49
disasm (state 2 = Change preloads id 0x12 at 0x6D937C, state 3 =
Defend preloads 0x13 at 0x6D9429). Dormant in practice: the command
grid never carries 0x12/0x13 (the .49 widget states own them and speak
their own correct literals in BattleMenuThread -- that path is
untouched), and they sat FIRST only as a patch over the shifted id-1
convention .102 removed. Fix: hardcoded names DEMOTED to last-resort
(after GKT and heap, before generic) and corrected to the file's own
words (0x12 Change / 0x13 Defend / 0x14 Limit -- "Change" also matches
the .49 state announce vocabulary, one-vocabulary rule). Limit in the
grid now resolves via GKT ([0x14]='Limit', same spoken text as the old
hardcoded case).

**3. Shadow source check (the regression net)**: when debug_log is on
and GKT resolves a command name, the heap section (where present =
2026-plain) resolves the same id at its own convention and any
disagreement logs "BMENU shadow diff id=0x%02X gkt=%ls heap=%ls".
The .102 bug would have printed "shadow diff id=0x01 gkt=Left
heap=Attack" in the FIRST battle's log -- machine-caught before any
human ear. A diff line means A SOURCE IS WRONG (convention drift or
heap rot), not necessarily the spoken one; the strings carried are the
evidence. Cost: one heap decode per cursor change, debug builds of the
log only. DLL literal check passed ("BMENU shadow diff" present).

**Standing rule recorded** (memory feedback-engine-convention-
verification + project memory): (a) any mod-side call into an engine
function ships only with each argument's convention byte-verified per
section/branch actually used (call-site sweep -- the
ff7_gkt_section5_callsites_static.py pattern); (b) every constant in
an engine-facing change is classified byte-verified (cite the script
log) or GUESSED -- guesses may not ship as primary and must appear in
the verify queue by name; (c) a source migration over a play-proven
source ships a debug-gated shadow comparison until the queue clears;
(d) layout questions about kernel data are answered by the positional
dump tool, never by signature-located copies.

**Verify queue [GKT102] extended (e)-(g)**: (e) zero "BMENU shadow
diff" lines in clean 2026-plain play (any hit = investigate
immediately); (f) Right on Attack still announces "Defend", Left
"Change" (unchanged .49 state path); (g) log header v2.30.103.

Built clean, deployed both installs, hash-verified 13D14A93D2133178.
NOT released.

---

### v2.30.104 (2026-08-09): magic menu "Spell 1" generics -- menu-side names migrated to GKT

**User report (log.12, v2.30.103 session)**: "Magic menu is back to
saying spell 1 rather than the spell name."

**Evidence -- the standing-stale-copy factory hit the MENU side, the
exact residual v2.30.100 predicted** ("menu-side sections still
heap-scanned; GKT migration = follow-up if menu junk ever reported").
Timeline in log.12: kernel2 scan latched magic=2F7C6258 at 19:10:11
(field, valid); battles 19:12-19:19 made FFNx free+reallocate its
kernel2 blocks (kernel2_reset_counters on reload -- the .100 root
cause); at 19:20:11 the magic menu's use-time head re-check RIGHTLY
rejected the stale pointer and every spell fell to the generic:
"MAGICM Magic slot=2 idx=0 id=0x00 'Spell 1, 5 MP'". No junk was
spoken (the .77/.100 guards did their job); the graceful fallback was
the audible symptom. Zero REJECT/AV/shadow lines -- the heap rejection
path is silent by design (null pointer, nothing to log).

**Convention check FIRST (the .103 rule, applied)**: piece 19 (magic
names) has NO filler entry -- [0]='Cure' [1]='Cure2' .. 256 entries
(ff7_kernel_text_tables_dump.py log; unlike piece 18's 'Left' filler,
each file has its OWN convention). The menu lists and the limit grid
carry raw magic-file indices (id 0x00 = Cure at 5 MP in log.12; limit
region 128+block*7), GKT section 0 = bias 0, cap 0xE0 covers all of
them, so heap and GKT agree on indexing: a pure source swap, no
convention change.

**Fix**: NEW MagicFileNameGktFirst(idx, src, out) = the single
menu-side magic-file name resolver -- game's own get_kernel_text
first (SEH-guarded, PlausibleActionName-gated, REJECT-logged), heap
section fallback, .103-style debug shadow comparison logging
"<src> shadow diff idx= gkt= heap=" when both resolve and disagree.
Migrated consumers: the magic menu spell/summon/E.Skill lists
(MAGICM) and LimitLevelLine's technique names (LIMITM). Magic
DESCRIPTIONS stay heap-scanned ([K2DESC]: GKT has no description
sections -- names only). DLL literal check passed
("shadow diff idx" present).

**Verify queue [MAGICM104]** (in PARKED.txt): (a) magic menu speaks
real spell names again (Cure at 5 MP etc.), also after a battle in
the same session (the stale-block window); (b) summon + E.Skill tabs
still named; (c) limit menu technique names still speak; (d) zero
"MAGICM/LIMITM shadow diff" lines in clean play; (e) log header
v2.30.104.
...
| `0x60C3A8`-region | field model init | sets the scale-based DEFAULTS: +0x72 collision = 30Â·scale>>9, +0x74 talk = 80Â·scale>>9 (e.g. scale 448 â†’ col 26, talk 70 â€” why most talk radii read 70). Same v2.30.26 hunt. âš  the talk-check READER of +0x74 was NOT found in three scan passes (word loads game-wide, +0x61 consumers) â€” the chirp/reach formula is behaviorally derived instead; and the ARRIVAL init 0x633FBC overrides the PLAYER's +0x72 to 34*scale>>9 on every field entry (v2.30.83) (see Â§8 v2.30.26) |
...
| `0x710DFA` | MAGIC menu sub (table[2]) | slotÂ·0x440+0xDBA5A0 spell-list resolve at 0x710F89 (= BATTLE_CHAR_BLOCK+0x108 â€” battle's list, stride 8 re-confirmed); 3-col index math + draw loop 0x711726..0x7117FB; usable classifier = u8[0x714440]â†’jump 0x714430 (color 7 white / 0 gray); kernel text via the 0x41963C getter; widget 0xDD1708, window struct 0xDD1690; pane cmp sites 0x710E0E/0x710FBA on 0x921100 (ff7_magic_menu_static.py, v2.30.48; 0x921100 semantics CORRECTED live v2.30.53-.59 = screen MODE, see the .data table row) |
...
| `0x921100` | MAGICMENU_MODE | u32 magic-screen mode: -1 entering, 0 tab selector, 1 TARGET pane, 2/3/4 = Magic/Summon/E.Skill lists (= TAB 0xDD169C + 2). LIVE-measured v2.30.53-.59 after the v2.30.48 static "pane 0/1" reading shipped broken three times (jump-table case indices are not stored values when writes carry an offset). Same 0x91E-0x925 per-screen state band |
...
| `0xDBA544` | battle LIMIT list (slot 0) | CHAR_BLOCK+0xAC parallel byte arrays: ids +0..+2, target flags +3..+5, count +6, 3 x 0x1C attack records +8; built from the savemap limit level by the menu module (v2.30.98; full layout in the S4 limit-list row) |
...
| `0xDC3C80` | second active-slot byte | fn[24]/the limit draw index the char + widget blocks from THIS copy while the widget init uses 0xDC3C7C -- equal whenever the battle menu is open; the mod keeps reading 0xDC3C7C (v2.30.98) |
...
### Magic menu block: 0xDD1690 - 0xDD17E8 (+ mode 0x921100 in the .data band)

Settled across v2.30.48-.59: static analysis gave the structure, live
measurement gave the semantics after three shipped-broken versions (the
"measure the memory" lesson, section 8 v2.30.54). Magic menu USER-CONFIRMED
complete 2026-08-01 (release v2.30.59).

| Address | Symbol | Notes |
|---------|--------|-------|
| `0xDD1690` | magic window struct ptr | +0x10/+0x12 screen coords (v2.30.48) |
| `0xDD169C` | MAGICMENU_TAB | 0=Magic 1=Summon 2=E.Skill (wraps at 3) -- the v2.30.54 delta-scan find; MAGICMENU_MODE 0x921100 = this + 2 while a list is open |
| `0xDD16D4` | MAGICMENU_TARGET_SLOT | target-pane (mode 1) party slot; the OK path feeds SAVEMAP_PARTY_IDS at 0x7137F8; play-confirmed v2.30.59 |
| `0xDD1708` / `0xDD1740` / `0xDD1778` | Magic / Summon / E.Skill list widgets | ctor 0x6F4D30 family (+0 col / +4 row / +0x14 scroll); per-character list bases = BATTLE_CHAR_BLOCK + 0x108 (3 cols) / + 0x2C8 (2 cols) / + 0x348 (2 cols) (v2.30.54) |
| `0xDD17E8` | MAGICMENU_PARTY_SLOT | u32 party slot 0..2 (x0x440 into the shared char-data block) (v2.30.48; section 4) |
